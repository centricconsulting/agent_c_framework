...
class SectionRegistry:
    ...
    def register_section_class(cls, section_class: Type[BasePromptSection], section_type: str = None) -> Type[BasePromptSection]:
        ...
        if section_type:
            ...
            if hasattr(section_class, 'model_fields') and 'section_type' in section_class.model_fields:
                ...
                if hasattr(field, 'default') and field.default is not None and field.default != PydanticUndefined:
    ...
    def is_section_registered(cls, section_type: str) -> bool:
        """Check if a section_type is registered"""
        registered = section_type in cls._model_registry or section_type in cls._sections
        if not registered:
            logger.warning(f"Section type '{section_type}' is not registered in SectionRegistry")
        return registered
