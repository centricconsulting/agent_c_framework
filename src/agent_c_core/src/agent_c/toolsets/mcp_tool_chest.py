...
class MCPToolChest(ToolChest):
    ...
    def load_config(self, config_path: str):
        ...
        with LoggingContext(operation="load_mcp_config", config_path=str(config_path)):
    ...
    def add_server(self, server_id: str, **server_config):
        ...
        """
        with LoggingContext(server_id=server_id):
            if server_id in self.mcp_servers:
                self.logger.warning("Replacing existing MCP server configuration",
    ...
    async def connect_server(self, server_id: str):
        ...
        with LoggingContext(server_id=server_id):
            if not server:
                self.logger.warning("No MCP server found",
            ...
            try:
                ...
                if success:
                    ...
                    self.logger.info("Added toolset for MCP server",
                                   toolset_name=toolset.__class__.__name__)
                else:
                    self.logger.warning("Failed to connect to MCP server",
                                       action="connect")

                return success
            except Exception as e:
                self.logger.error("Error connecting to MCP server",
                                 error=str(e), action="connect")
                return False
    ...
    async def connect_servers(self):
        ...
        """
        with LoggingContext(operation="mcp_server_connections", server_count=len(self.mcp_servers)):
            connection_results = {}
            connection_tasks = []
            ...
            for server_id, task in connection_tasks:
                try:
                    ...
                    self.logger.error("Error handling connection to MCP server", 
                                    server_id=server_id, error=str(e), action="connect")
    ...
    async def disconnect_server(self, server_id: str):
        ...
        with LoggingContext(server_id=server_id):
            ...
            try:
                ...
                self.logger.error("Error disconnecting from MCP server",
                                error=str(e), action="disconnect")
                return False
    ...
    async def disconnect_servers(self):
        ...
        """
        with LoggingContext(operation="mcp_server_disconnections"):
            ...
            for server_id, task in disconnection_tasks:
                try:
                    ...
                    self.logger.error("Error handling disconnection from MCP server", 
                                    server_id=server_id, error=str(e), action="disconnect_async")
    ...
    async def init_tools(self, **kwargs):
        ...
        if not self._ToolChest__essential_toolset_names or essential_has_mcp:
            ...
            self.logger.info("Initialized MCP servers and tools", 
            ...
            if failed > 0:
                failed_servers = [server_id for server_id, success in connection_results.items() if not success]
                self.logger.warning("Some MCP servers failed to connect", 
                                  failed_count=failed, 
    ...
    async def shutdown(self):
        ...
        self.logger.info("Shutting down MCP servers", 
                       ...
                       connected_servers=[s_id for s_id, server in self.mcp_servers.items() if server.connected])
