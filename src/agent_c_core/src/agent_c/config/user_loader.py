...
class UserLoader(ConfigLoader):
    ...
    def load_user_dict(self, uid: str, force_reload: bool = False, restore_deleted: bool = False) -> Dict[str, Any]:
        """
        Load a user by their ID from a JSON file.

        Args:
            uid: The ID of the user to load
        ...
        Raises:
            ...
            ValidationError: If the JSON doesn't match the expected schema
        """
        user_id: str = uid.strip().lower()
        if user_id in self._user_cache and not force_reload:
            return self._user_cache[user_id].model_dump()
    ...
    def load_user_id(self, uid: str, force_reload: bool = False, restore_deleted: bool = False) -> 'ChatUser':
        user_id: str = uid.strip().lower()
        if user_id in self._user_cache and not force_reload:
            return self._user_cache[user_id]

        user_data = self.load_user_dict(user_id, force_reload=force_reload, restore_deleted=restore_deleted)
