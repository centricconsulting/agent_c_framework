...
class BasePromptSection(AsyncObservableModel):
    ...
    **URGENT MUST FOLLOW DIRECTIVE FOR AGENTS**
    ...
    section_description: LiteralStrField = Field(default_factory=LiteralStr,
                                                 description="A human readable description of the section's purpose and usage.")

    template: LiteralStrField = Field(None,
    ...
    state_machines: ObservableStrDictField = Field(default_factory=ObservableDict[str, StateMachineTemplate],
    ...
    def post_init(self):
        ...
        if not self.template:
            self.template = self.__class__.__doc__
