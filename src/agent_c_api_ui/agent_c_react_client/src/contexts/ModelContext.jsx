...
useEffect(() => {
    ...
    // Remove isStorageAvailable since we're using the proper storageService
    // which has built-in error handling
    ...
    /**
     * Save model config to localStorage with improved error handling
     */
    const saveConfigToStorage = (updatedConfig = {}) => {
        try {
            ...
            logger.error('Failed to save model configuration', 'ModelContext', {
                error: err.message,
                stack: err.stack,
                name: err.name,
                errorType: err.constructor ? err.constructor.name : typeof err
            });
        }
    };
    ...
    const loadSavedConfig = () => {
        try {
            ...
            logger.error('Error loading saved configuration', 'ModelContext', {
                error: err.message,
                stack: err.stack
            });
    ...
    const changeModel = async (newModelName) => {
        try {
            ...
            logger.error('Failed to change model', 'ModelContext', {
                error: error.message,
                stack: error.stack
            });
    ...
    const initializeWithModel = async (model = null) => {
        try {
            ...
            logger.error('Failed to initialize model', 'ModelContext', {
                error: error.message,
                stack: error.stack
            });
    ...
    useEffect(() => {
        ...
        const initModels = async () => {
            try {
                ...
                logger.error('Failed to initialize models', 'ModelContext', {
                    error: error.message,
                    stack: error.stack
                });
