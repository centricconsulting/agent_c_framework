...
Imports Diamond.Common.ChoicePoint.Library.LowLevelRecords

Public Class ctl_UnderwritingQuestionsByLob
    Inherits VRControlBase

    Private ReadOnly _flags As IEnumerable(Of IFeatureFlag)
    Private _uwQuestionSvc As IUnderwritingQuestionsService
    ...
    Protected ReadOnly Property UwQuestionSvc As IUnderwritingQuestionsService
        Get
            If _uwQuestionSvc Is Nothing AndAlso
               Me.Quote?.LobType IsNot Nothing Then
                _uwQuestionSvc = UnderwritingQuestionServiceFactory.BuildFor(Me.Quote.LobType)
            End If

            Return _uwQuestionSvc
        End Get

    End Property

    Public Sub New()
        _flags = New List(Of IFeatureFlag)
        '_uwQuestionSvc = UnderwritingQuestionServiceFactory.BuildFor(Me.Quote.LobType)
    End Sub
    ''' <summary>
...
#Region "Public Events"
    Public Event UnderwritingQuestionsSaved(ByVal sender As Object, ByVal QuoteID As String)
    Public Event UnderwritingQuestionsSaveFailed(ByVal sender As Object, ByVal QuoteID As String)
    Public Event UnderwritingQuestionsLoadFailed(ByVal sender As Object, ByVal ErrMsg As String)
    ...
    Public Event UnderwritingQuestionsSaved_Endorsements(ByVal sender As Object, ByVal PolicyId As Integer, ByVal PolicyImageNum As Integer)
    Public Event UnderwritingQuestionsSaveFailed_Endorsements(ByVal sender As Object, ByVal PolicyId As Integer, ByVal PolicyImageNum As Integer)
...
#Region "UI Support"
    ...
    Private Sub InitializeQuestionData()
        ...
        Dim request As New UnderwritingQuestionRequest() With {
                            ...
                            .Quote = Quote,
                            .TypeFilter = UnderwritingQuestionRequest.QuestionTypeFilter.ExcludeUnmapped Or
                                          UnderwritingQuestionRequest.QuestionTypeFilter.MinimumEffectiveDate,
                            .MinimumEffectiveDate = CDate(Quote.EffectiveDate)
                            }

        Try

            Dim result = UwQuestionSvc.GetQuestions(request)
            Questions = Nothing

            If result IsNot Nothing AndAlso result.Any Then
                Questions = result

                'simplify UI handling by updating question properties
                Questions.ForEach(Sub(q)
                                      q.ShowDescriptionOnNo = (q.AlwaysShowDescription OrElse q.ShowDescriptionOnNo) AndAlso Not q.NeverShowDescription
                                      q.ShowDescriptionOnYes = (q.AlwaysShowDescription OrElse q.ShowDescriptionOnYes) AndAlso Not q.NeverShowDescription
                                  End Sub)

                rptUWQ.DataSource = Questions.Where(Function(q) q.HideFromDisplay = False AndAlso q.NeverShowQuestions = False)
                rptUWQ.DataBind()
            Else
...
#Region "Guards"
    ...
    Protected Function Attempt_SaveAnswersToQuote() As Boolean
        ...
        Try
            ...
            retval = UwQuestionSvc?.SaveAnswers(saveRequest)
...
#Region "Event Handlers"
        ...
        Try
            ...
            ElseIf Attempt_SaveAnswersToQuote() = True Then
                ...
                If IsQuoteEndorsement() Then
                    RaiseEvent UnderwritingQuestionsSaved_Endorsements(Me, EndorsementPolicyId, EndorsementPolicyImageNum)
                ...
                Else 'new business
                    RaiseEvent UnderwritingQuestionsSaved(Me, QuoteId)
                    If e.CommandName = COMMAND_APP Then RaiseEvent RequestNavigationToApplication(Me, QuoteId)
                End If

            ElseIf IsQuoteEndorsement() Then
                RaiseEvent UnderwritingQuestionsSaveFailed_Endorsements(Me, EndorsementPolicyId, EndorsementPolicyImageNum)
            Else 'new business
                RaiseEvent UnderwritingQuestionsSaveFailed(Me, QuoteId)
            End If
    ...
    Protected Sub RepeaterRadioButton_CheckedChanged(sender As Object, e As EventArgs)
        ...
        If changedQuestion IsNot Nothing Then
            ...
            If questionValue = "Yes" And rbSelected.Checked Then
                changedQuestion.QuestionAnswerYes = True
                changedQuestion.QuestionAnswerNo = False
                changedQuestion.DetailText = IIf(changedQuestion.AlwaysShowDescription OrElse
                                                 (changedQuestion.ShowDescriptionOnYes AndAlso
                                                  changedQuestion.NeverShowDescription = False), txtDetail.Text, String.Empty)
            ElseIf questionValue = "No" And rbSelected.Checked Then
                changedQuestion.QuestionAnswerYes = False
                changedQuestion.QuestionAnswerNo = True
                changedQuestion.DetailText = IIf(changedQuestion.AlwaysShowDescription OrElse
                                                 (changedQuestion.ShowDescriptionOnNo AndAlso
                                                  changedQuestion.NeverShowDescription = False), txtDetail.Text, String.Empty)
            End If
    ...
    Protected Sub RepeaterTextbox_TextChanged(sender As Object, e As EventArgs)
        ...
        If changedQuestion IsNot Nothing Then
            ...
            changedQuestion.DetailText = txtChanged.Text
            Questions = questionsLocal
        End If
    End Sub
