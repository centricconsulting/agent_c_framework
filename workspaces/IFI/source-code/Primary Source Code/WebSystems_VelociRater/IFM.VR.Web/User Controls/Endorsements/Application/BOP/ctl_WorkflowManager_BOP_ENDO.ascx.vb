...
Public Class ctl_WorkflowManager_BOP_ENDO
        ...
        If IsQuoteEndorsement() Then
            If workflow <> Common.Workflow.Workflow.WorkflowSection.summary AndAlso workflow <> Common.Workflow.Workflow.WorkflowSection.fileUpload Then
                ...
                Select Case TypeOfEndorsement()
                    Case EndorsementStructures.EndorsementTypeString.BOP_AmendMailing
                        workflow = Common.Workflow.Workflow.WorkflowSection.policyHolders
                    Case EndorsementStructures.EndorsementTypeString.BOP_AddDeleteLocation, EndorsementStructures.EndorsementTypeString.BOP_AddDeleteLocationLienholder
                        workflow = Common.Workflow.Workflow.WorkflowSection.location
                    Case EndorsementStructures.EndorsementTypeString.BOP_AddDeleteContractorsEquipment, EndorsementStructures.EndorsementTypeString.BOP_AddDeleteContractorsEquipmentLienholder
                        workflow = Common.Workflow.Workflow.WorkflowSection.coverages 'App Page 1 section
                End Select
            End If
        End If
    ...
    Public Overrides Sub SetCurrentWorkFlow(workflow As Workflow.WorkflowSection, subWorkFlowIndex As String)
        ...
        Select Case workflow
            ...
            Case Common.Workflow.Workflow.WorkflowSection.printFriendly  'Updated 09/21/2021 for BOP Endorsements Task 61506, changed from drivers to printFriendly
                ...
                Exit Select
            Case IFM.VR.Common.Workflow.Workflow.WorkflowSection.coverages
                Me.ctl_BOP_ENDO_PolicyLevelCoverages.Visible = True
                Me.ctl_BOP_ENDO_PolicyLevelCoverages.Populate()
                Exit Select
            Case Else
                Exit Select
        End Select
...
#End If
    ...
    Public Overrides Sub ValidateControl(valArgs As VRValidationArgs)
        Select Case TypeOfEndorsement()
            ...
            Case EndorsementStructures.EndorsementTypeString.BOP_AddDeleteLocationLienholder
                Me.ctl_BOP_ENDO_LocationList.ValidateControl(valArgs)
            Case EndorsementStructures.EndorsementTypeString.BOP_AddDeleteContractorsEquipmentLienholder
                Me.ctl_BOP_ENDO_PolicyLevelCoverages.ValidateControl(valArgs)
        End Select
    ...
    Protected Overrides Sub RateWasRequested()
        ...
        ctl_BOP_ENDO_LocationList.CheckTranactionLimitAtRate()
    ...
    Public Overrides Function Save() As Boolean
        Select Case TypeOfEndorsement()
            Case EndorsementStructures.EndorsementTypeString.BOP_AddDeleteLocationLienholder
                ctl_BOP_ENDO_LocationList.Save()
            Case EndorsementStructures.EndorsementTypeString.BOP_AddDeleteContractorsEquipmentLienholder
                ctl_BOP_ENDO_PolicyLevelCoverages.Save()
            Case EndorsementStructures.EndorsementTypeString.BOP_AmendMailing
                ctlIsuredList.Save()
            Case Else
...
#Region "Save Requests"
    ...
    Private Sub ctl_BOP_PolicyLevelCoverages_SaveRequested(args As VrControlBaseSaveEventArgs) Handles ctl_BOP_ENDO_PolicyLevelCoverages.SaveRequested
        Me.ControlsToValidate_Custom.Add(Me.ctl_BOP_ENDO_PolicyLevelCoverages)
    End Sub

    Private Sub ctl_BOP_LocationList_SaveRequested(args As VrControlBaseSaveEventArgs) Handles ctl_BOP_ENDO_LocationList.SaveRequested
        Me.ControlsToValidate_Custom.Add(Me.ctl_BOP_ENDO_LocationList)
    End Sub

    Private Sub ctl_TreeView_Update() Handles ctl_BOP_ENDO_LocationList.CallTreeUpdate
        ctlTreeView.RefreshQuote()
    End Sub
