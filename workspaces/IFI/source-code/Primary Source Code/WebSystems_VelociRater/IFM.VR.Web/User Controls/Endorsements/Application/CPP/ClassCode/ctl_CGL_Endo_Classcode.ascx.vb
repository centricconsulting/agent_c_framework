...
Public Class ctl_CGL_Endo_Classcode
    ...
    Private ReadOnly Property IsMyClassCodePreExisting As Boolean
        Get
            If AllPreExisting IsNot Nothing AndAlso MyClassCode IsNot Nothing Then
                Dim ClassCodeLocation As String
                Dim ClassCodeAssignment As Integer
                If IsAtLocationLevel Then
                    ClassCodeLocation = ddAssignmentLocation.SelectedItem.ToString
                    ClassCodeAssignment = 2
                Else
                    ClassCodeLocation = String.Empty
                    ClassCodeAssignment = 1
                End If

                If AllPreExisting.PreExisting_CglClassCodes.isPreExistingClassCodes(MyClassCode.ClassCode, ClassCodeAssignment, ClassCodeLocation) Then
                    Return True
                End If
            End If
    ...
    Public Overrides Sub AddScriptWhenRendered()
        ...
        Else
            ...
            Me.VRScript.CreateJSBinding(Me.btnSearch, ctlPageStartupScript.JsEventType.onclick, "VrGlClassCodes.PerformSearch(" & HttpContext.Current.Items("ccUiBindingIndex").ToString() & ",$('#" & Me.ddSearchType.ClientID & "').val(),$('#" & Me.txtSearchClassCode.ClientID & "').val(),'" & CInt(Me.Quote.LobType).ToString() & "','" & If(Me.Quote.ProgramTypeId.IsNumeric = False, "54", Me.Quote.ProgramTypeId) & "','" & Me.divSearchResults.ClientID & "','False','" & isClassCodeAssignmentAvailable.ToString().ToLower() & "'); return false;")
    ...
    Public Overrides Sub Populate()
        ...
        If IsAtLocationLevel Then
            ...
            If ClassCodeAssignmentHelper.IsClassCodeAssignmentAvailable(Quote) AndAlso IsMyClassCodePreExisting = False Then
                'For New Class Codes on Endorsements - Force Class Code Assignment drop down to Location and disable if Premium Exposure Description begins with "Area". Full description would be "Area, Products/Completed Operations". Sometimes this comes back truncated at Opera or Op. Do not know if there are more variations.
        ...
        Else
            ...
            If IsMyClassCodePreExisting = False Then
                ddAssignment.Enabled = True
            End If
        ...
        If IsMyClassCodePreExisting Then
            DisableControls()
            HideRemoveButton()
        End If

        Dim EndoCppHelper = New EndorsementsCppHelper(Quote)
        lnkBtnAdd.Visible = EndoCppHelper.ShowAddClassCodeLink
