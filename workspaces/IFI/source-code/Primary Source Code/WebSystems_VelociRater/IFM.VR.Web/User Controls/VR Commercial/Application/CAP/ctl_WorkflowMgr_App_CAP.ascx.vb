...
Partial Class ctl_WorkflowMgr_App_CAP
    ...
    Public Overrides Sub HandleStartUpWorkFlowSelection(workflow As Workflow.WorkflowSection)
        ...
        Select Case workflow
            ...
            Case Common.Workflow.Workflow.WorkflowSection.printFriendly
                SetCurrentWorkFlow(Common.Workflow.Workflow.WorkflowSection.printFriendly, "")  ' This enum used for the Print-Friendly summary MGB
                Exit Select
            Case Else
                Exit Select
        End Select
    ...
    Public Overrides Function Save() As Boolean
        ...
        If _NeedsToRepopulateVehicleList = True Then 'added 9/22/2017
            _NeedsToRepopulateVehicleList = False
            Me.ctl_AppSection_CAP.PopulateVehicleList()
        End If
        Return True
    End Function
    ...
    Public Overrides Sub ValidateControl(valArgs As VRValidationArgs)
        ...
        'Me.ctlCommercialUWQuestionList.ValidateControl(valArgs)
        'Me.ctl_AttachmentUpload.ValidateControl(valArgs)
        'Me.ctl_CAP_QuoteSummary.ValidateControl(valArgs)
        'Me.ctlCommercial_IRPM.ValidateControl(valArgs)
        'Me.ctlCommercial_DocPrint.ValidateControl(valArgs)
        'Me.ctl_AppSection_CAP.ValidateControl(valArgs)
        ...
        If validateEffectiveDate Then ' you check this because you only want to validate if it is coming from the btnFinalRate_Click() below - all other times you do no validate the effective date because it isn't even on the screen
            ...
            Dim valList2 = IFM.VR.Validation.ObjectValidation.CommLines.LOB.CAP.PolicyLevelValidations.ValidatePolicyLevel(Me.Quote, valArgs.ValidationType)
...
#Region "Workflows"
    ...
    Public Sub HideAllControls()
        For Each cntrl As VRControlBase In Me.ChildVrControls
            cntrl.Visible = False
        Next
        Me.ctl_ReturnToQuoteSide1.Visible = True
    End Sub
    ...
    Public Overrides Sub SetCurrentWorkFlow(workflow As IFM.VR.Common.Workflow.Workflow.WorkflowSection, subWorkFlowIndex As String)
        ...
        Select Case workflow
            Case IFM.VR.Common.Workflow.Workflow.WorkflowSection.app
                Me.ctl_AppSection_CAP.Visible = True
    ...
    Private Sub ctl_WorkflowManager_App_CAP_BroadcastWorkflowChangeRequestEvent(type As Workflow.WorkflowSection, subworkflowParm As String) Handles Me.BroadcastWorkflowChangeRequestEvent
        Select Case type
...
#Region "Save Requests"
    ...
    Private Sub ctl_AppSection_CAP_App_Rate_ApplicationRatedSuccessfully() Handles ctl_AppSection_CAP.App_Rate_ApplicationRatedSuccessfully
    ...
    Private Sub ctl_AppSection_CAP_QuoteRated() Handles ctl_AppSection_CAP.QuoteRated
        ctlTreeView.RefreshRatedQuote()
    End Sub

    Private Sub ctl_AppSection_CAP_SaveRequested(args As VrControlBaseSaveEventArgs) Handles ctl_AppSection_CAP.SaveRequested
        Me.ControlsToValidate_Custom.Add(ctlCommercialUWQuestionList)
        Me.ControlsToValidate_Custom.Add(ctl_AttachmentUpload)
        Me.ControlsToValidate_Custom.Add(ctl_CAP_QuoteSummary)
        Me.ControlsToValidate_Custom.Add(ctlCommercial_IRPM)
        Me.ControlsToValidate_Custom.Add(ctlCommercial_DocPrint)
        Me.ControlsToValidate_Custom.Add(ctl_AppSection_CAP)
    End Sub
...
#Region "Tree Events"
    ...
    Private Sub ctlTreeView_ShowApplication(sender As Object, e As EventArgs) Handles ctlTreeView.ShowApplication
        ...
        If Me.Quote IsNot Nothing Then
            ...
            If Me.SubQuoteFirst.PolicyUnderwritings IsNot Nothing Then
                For Each q In Me.SubQuoteFirst.PolicyUnderwritings
                ...
                If Unanswered Then
                    ...
                    Me.ValidationHelper.AddError("You must complete the Underwriting Questions before proceeding.")
                Else
                    ...
                    Select Case Quote.LobType
                        Case QuickQuote.CommonObjects.QuickQuoteObject.QuickQuoteLobType.CommercialAuto
                            'If Quote.PolicyUnderwritings IsNot Nothing AndAlso Quote.PolicyUnderwritings.Count > 6 Then ' CAP has 6 kill questions...
                            'updated 8/13/2018 to use 1st stateQuote instead of main quote
                            If SubQuoteFirst.PolicyUnderwritings IsNot Nothing AndAlso SubQuoteFirst.PolicyUnderwritings.Count > 6 Then ' CAP has 6 kill questions...
    ...
    Private Sub ctl_AppSection_CAP_AIChanged() Handles ctl_AppSection_CAP.AIChanged 'added 9/22/2017
        If _IsSaving = True Then
            _NeedsToRepopulateVehicleList = True
        Else
            _NeedsToRepopulateVehicleList = False
            'this logic was previously in ctl_AppSection_CAP.HandleAIChange; not sure if Save is needed, but this likely won't get hit since the AI updated events should fire during save, and populate will happen after if needed (see Save code above)
    ...
    '    Me.ctlTreeView.RatedQuoteObject = DirectCast(Me.Page.Master, VelociRater).GetRatedQuotefromCache()
    'End Sub
