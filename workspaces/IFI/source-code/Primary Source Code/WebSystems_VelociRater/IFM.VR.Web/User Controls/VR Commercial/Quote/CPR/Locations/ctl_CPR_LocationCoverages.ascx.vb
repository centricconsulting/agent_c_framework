...
Public Class ctl_CPR_LocationCoverages
    ...
    Public Overrides Sub Populate()
        ...
        If Quote IsNot Nothing Then
            If MyLocation IsNot Nothing Then
                ' WIND/HAIL
                'Updated 12/02/2021 for CPP Endorsements Task 67023 MLW, updated 3/15/2022 for CPP Endorsements Task 73859 MLW
                If IsQuoteReadOnly() Then
                    If WindHailDefaultingHelper.IsWindHailDefaultingAvailable(Quote) Then
                        tdWindHailDeductible.Attributes.Add("style", "display:''")
                        tdWindHailDeductInfo.InnerText = "Wind/Hail deductibles are applied per building in the event of a loss."
                    Else
                        tdWindHailDeductInfo.InnerText = "Refer to the policy declaration page or change summary for any Wind Hail Deductible % that applies to coverages at this location."
                        tdWindHailDeductible.Attributes.Add("style", "display:none")
                    End If
                    ''    WebHelper_Personal.SetDropDownValue_ForceDiamondValue(ddWindHailDeductible, MyLocation.WindHailDeductibleLimitId, QuickQuoteClassName.QuickQuoteBuilding, QuickQuotePropertyName.WindHailDeductibleLimitId)
                    'lblWindHailDeductible.Text = "Wind Hail Deductible"
                    ''Else
                    ''    SetFromValue(ddWindHailDeductible, MyLocation.WindHailDeductibleLimitId, "0")
                End If
                PopulateWindHailDropDown()
                If RemoveBuildingLevelDeductibleHelper.IsRemoveBuildingLevelDeductibleAvailable(Quote) Then
                    If IsQuoteReadOnly() Then
    ...
    Public Overrides Function Save() As Boolean
        ...
        If MyLocation IsNot Nothing Then
            ...
            If SubQuoteFirst.HasBlanketBuilding OrElse SubQuoteFirst.HasBlanketBuildingAndContents OrElse SubQuoteFirst.HasBlanketBusinessIncome OrElse SubQuoteFirst.HasBlanketContents Then
                hasBlanket = True
                MyLocation.DeductibleId = BlanketHelper_CPR_CPP.GetBlanketDeductibleID(SubQuoteFirst)
                MyLocation.CauseOfLossTypeId = BlanketHelper_CPR_CPP.Get_SelectedBlanketCauseOfLossProperty(SubQuoteFirst)
                MyLocation.CoinsuranceTypeId = BlanketHelper_CPR_CPP.Get_SelectedBlanketcoInsuranceProperty(SubQuoteFirst)
                MyLocation.ValuationMethodTypeId = BlanketHelper_CPR_CPP.Get_SelectedBlanketValuationProperty(SubQuoteFirst)
            End If
    ...
    Private Function GetLocationDeductibleId() As String
        ...
        If MyLocation.Buildings IsNot Nothing AndAlso MyLocation.Buildings.Count > 0 Then
            ...
            If dedId = "" Then
                dedId = "9"  ' default to 500 changed to 1000 per task 62836
                dedVal = 1000
            Else
    ...
    Protected Sub lnkClear_Click(sender As Object, e As EventArgs) Handles lnkClear.Click
        ...
        If RemoveBuildingLevelDeductibleHelper.IsRemoveBuildingLevelDeductibleAvailable(Quote) Then
            If ddLocationPropertyDeductible.Items IsNot Nothing AndAlso ddLocationPropertyDeductible.Items.Count > 0 Then
                ddLocationPropertyDeductible.SelectedValue = "9"
            End If
        End If
        chkEquipmentBreakdown.Checked = False
    End Sub
    ...
    Public Sub PopulateWindHailDropDown()
        SetFromValue(ddWindHailDeductible, MyLocation.WindHailDeductibleLimitId, "0")
    End Sub
