...
Public Class ctl_App_AdditionalInsureds_CGL
    ...
    Private _PremiumPerAInsured As String = "25"
    ...
    Public Overrides Sub Populate()
        ...
        If Me.Quote.AdditionalInsureds IsNot Nothing AndAlso Me.Quote.AdditionalInsureds.Count > 0 OrElse Me.Quote.AdditionalInsuredsManualCharge <> "" Then
            ...
            For Each ai As QuickQuoteAdditionalInsured In Me.Quote.AdditionalInsureds
                ...
                If ai.AdditionalInsuredType = 21021 AndAlso ai.CoverageCodeName = "Vendors" AndAlso ai.Description.ToLower <> "n/a" Then
                ...
                End If
                'If String.IsNullOrEmpty(ai.ManualPremiumAmount) Then ai.ManualPremiumAmount = "25"
    ...
    Public Overrides Function Save() As Boolean
        ...
        If Me.Visible Then
            ...
            dt = CType(Me.Session.Item("DTadditionalInsured"), DataTable)
            Dim total = 0
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                For Each dtrow As DataRow In dt.Rows
                    ...
                    ai.NameOfPersonOrOrganization = dtrow.Item("Name").ToString
                    ai.DesignationOfPremises = dtrow.Item("DesigPrem").ToString
                    ai.ProductDescription = dtrow.Item("Products").ToString
                    ai.Description = "n/a"
                    ...
                    'Below is not supported for CGL
    ...
    Protected Sub DropDownList_additonalInsuredType_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles DropDownList_additonalInsuredType.SelectedIndexChanged
        ...
        'Me.addIpremisesROW.Disabled = False
        Me.TextBox_addIdesignation.Enabled = True
    ...
    Private Sub saveAdditionalInsured()
        If CBool(Me.Session.Item("isEditing")) Then
            Me.ValidateAIEdit(New VRValidationArgs)
        End If

        Dim DTadditionalInsured As DataTable
        DTadditionalInsured = CType(Me.Session.Item("DTadditionalInsured"), DataTable)
        ...
        If DTadditionalInsured IsNot Nothing Then
            ...
            ' We are defaulting & disabling this text box on 21024's.  Bug 24472.
            ' Disabled text boxes do not return a value.
            If (Me.DropDownList_additonalInsuredType.SelectedValue = "21024") Then
                Me.TextBox_addIdesignation.Text = "Any job sites where work is performed by the named insured for the designated additional insured."
            End If

            If Me.Session.Item("isEditing") IsNot Nothing AndAlso CBool(Me.Session.Item("isEditing")) = True _
                AndAlso Me.Session.Item("additionalInsuredRowID") IsNot Nothing AndAlso Me.Session.Item("additionalInsuredRowID").ToString <> "" AndAlso Not Me.ValidationHelper.HasErrros Then

                Dim dr As DataRow = DTadditionalInsured.Rows(CInt(Me.Session.Item("additionalInsuredRowID")))
    ...
    Protected Function GetAIObject(ClassCodeID As QuickQuote.CommonObjects.QuickQuoteAdditionalInsured.QuickQuoteAdditionalInsuredType, Optional count As Integer = 0) As QuickQuote.CommonObjects.QuickQuoteAdditionalInsured
        ...
        ai.CoverageCodeId = ClassCodeID.ToString
    ...
    Public Sub UpdateAIEditBox()
        ...
        Me.addIbuttonROW.Visible = False
        DropDownList_additonalInsuredType.SetFromValue("0")
        Me.DropDownList_additonalInsuredType.Enabled = True
        Me.additionalInsuredTYPErow.Visible = True
    ...
    Protected Function ValidateByType(dtrow As DataRow, editorValidation As Boolean) As Boolean
        ...
        Dim Name = dtrow.Item("Name").ToString
        Dim Prem = dtrow.Item("DesigPrem").ToString
        Dim Products = dtrow.Item("Products").ToString
        ...
        Select Case TypeNumber
            Case 0
                'If TypeNumber Is Nothing OrElse String.IsNullOrEmpty(TypeNumber) = True Then
                errorMsg = "Missing Insured Type"
                'End If
                If errorMsg IsNot Nothing AndAlso String.IsNullOrEmpty(errorMsg) = False Then
                    If editorValidation AndAlso CBool(Me.Session.Item("isEditing")) Then
                        Me.ValidationHelper.AddError(errorMsg, Me.DropDownList_additonalInsuredType.ClientID)
                        hasError = True
                    Else
        ...
        Select Case TypeNumber
            ...
            Case "21024"
                ' We are defaulting & disabling this text box on 21024's.  Bug 24472.
                ' Disabled text boxes do not return a value, so no check needed for this type.
                ' ============================================================================
                'If Prem Is Nothing OrElse String.IsNullOrEmpty(Prem) = True Then
