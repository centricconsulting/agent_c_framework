...
Public Class ctl_CPR_Building
    ...
    Public ReadOnly Property BlanketDeductibleId As String
        Get
            Dim defaultBlankedLimitId As String = "9"
            'Updated 12/20/18 for multi state bug 30442 MLW
            If SubQuoteFirst IsNot Nothing Then
                If RemoveBuildingLevelDeductibleHelper.IsRemoveBuildingLevelDeductibleAvailable(Quote) Then
                    Dim deductibleId = BlanketHelper_CPR_CPP.GetBlanketDeductibleID(SubQuoteFirst)
                    If deductibleId IsNot Nothing AndAlso deductibleId <> "0" Then
                        Return deductibleId
                    Else
                        Return defaultBlankedLimitId
                    End If
                    'If HasBlanketAppliedChecked() AndAlso Quote.Locations.HasItemAtIndex(0) Then
                    '    If Quote.Locations(0).DeductibleId <> "" Then Return Quote.Locations(0).DeductibleId Else Return defaultBlankedLimitId  ' 8 = 500 changed to 1000 per task 62836
                    'Else
                    '    Return defaultBlankedLimitId
                    'End If
                Else
                    'If Quote.HasBlanketBuilding OrElse Quote.HasBlanketBuildingAndContents Then
                    If SubQuoteFirst.HasBlanketBuilding OrElse SubQuoteFirst.HasBlanketBuildingAndContents Then
                        If Quote.Locations.HasItemAtIndex(0) AndAlso Quote.Locations(0).Buildings.HasItemAtIndex(0) Then
                            If Quote.Locations(0).Buildings(0).DeductibleId <> "" Then Return Quote.Locations(0).Buildings(0).DeductibleId Else Return defaultBlankedLimitId  ' 8 = 500 changed to 1000 per task 62836
                        Else
                            Return defaultBlankedLimitId   ' 8 = 500 changed to 1000 per task 62836
                        End If
                        'Updated 12/20/18 for multi state bug 30442 MLW
                        'ElseIf Quote.HasBlanketContents Then
                    ElseIf SubQuoteFirst.HasBlanketContents Then
                        If Quote.Locations.HasItemAtIndex(0) AndAlso Quote.Locations(0).Buildings.HasItemAtIndex(0) Then
                            If Quote.Locations(0).Buildings(0).PersPropCov_DeductibleId <> "" Then Return Quote.Locations(0).Buildings(0).PersPropCov_DeductibleId Else Return defaultBlankedLimitId  ' 8 = 500 changed to 1000 per task 62836
                        Else
                            Return defaultBlankedLimitId  ' 8 = 500 changed to 1000 per task 62836
                        End If
                    Else
    ...
    Public ReadOnly Property LocationPropertyDeductibleClientID As String
        Get
            Dim cId As String = ""
            If Me.ParentVrControl IsNot Nothing AndAlso TypeOf Me.ParentVrControl Is ctl_CPR_BuildingList Then
                Dim bl As ctl_CPR_BuildingList = CType(Me.ParentVrControl, ctl_CPR_BuildingList)
                If bl.ParentVrControl IsNot Nothing AndAlso TypeOf bl.ParentVrControl Is ctl_CPR_Location Then
                    cId = CType(bl.ParentVrControl, ctl_CPR_Location).LocationPropertyDeductibleClientID
                End If
            End If
            Return cId
        End Get
    ...
    Public Overrides Sub LoadStaticData()
        If ddINFConstruction.Items Is Nothing OrElse ddINFConstruction.Items.Count <= 0 Then
            ...
            '' Deductible - moved to LoadBuildingCoverageDeductibles()
            ...
            If ValuationACVHelper.IsValuationACVAvailable(Quote) Then
                If MyBuilding IsNot Nothing Then
                    '1=Replacement Cost
                    If ValuationACVHelper.RemoveReplacementCostForDwellingClass(MyBuilding) Then
            ...
            LoadBuildingCoverageDeductibles()

            'Functional Replacement Cost = 7
            If IsQuoteReadOnly() = False AndAlso MyBuilding IsNot Nothing AndAlso ValuationACVHelper.RemoveReplacementCostForDwellingClass(MyBuilding) Then
                'Reusing RemoveReplacementCostForDwellingClass(MyBuilding) because Functional Replacement Cost is using the same requirements as Replacement Cost
    ...
    Public Overrides Sub Populate()
        ...
        trYearBuilt.Attributes.Add("style", "display:none")
        trSquareFeet.Attributes.Add("style", "display:none")
        ...
        'Updated 12/08/2021 for CPP Endorsements Task 67023 MLW
        If Not IsQuoteReadOnly() Then
            ...
            ' Risk Type
            Select Case MyBuilding.ClassificationCode.ClassCode
            ...
            ' Deductibles
            If RemoveBuildingLevelDeductibleHelper.IsRemoveBuildingLevelDeductibleAvailable(Quote) = False Then
                IFM.VR.Web.Helpers.WebHelper_Personal.SetdropDownFromValue(ddBCDeductible, "9")    ' Default is Building Coverage Deductible
                IFM.VR.Web.Helpers.WebHelper_Personal.SetdropDownFromValue(ddPPCDeductible, "9")    ' Default is Personal Property Coverage Deductible
                IFM.VR.Web.Helpers.WebHelper_Personal.SetdropDownFromValue(ddPPODeductible, "9")    ' Default is Personal Property of Others Deductible
            End If
            'End Defaults
        End If
        ...
        If HasCoverage(CPRBuildingCoverageType.BuildingCoverage) Then
            ...
            PopulateInflationGuard() 'Added 10/20/2022 for task 77527 MLW
            'moved to new sub PopulateInflationGuard()
            ...
            '            'Updated 10/20/2022 for task 77527 MLW
            '            If CPR_InflationGuardHelper.IsInflationGuardNo2Available(Quote) Then
            '                ddBCInflationGuard.SelectedValue = "2"
            '            Else
            ...
            ''End If
            ''Updated 12/02/2021 for CPP Endorsements Task 67023 MLW
            'If Not ddBCInflationGuard.Items.FindByValue("0") Is Nothing AndAlso Not IsQuoteReadOnly() Then
            '    ddBCInflationGuard.Items.Remove("N/A")
            'End If
            ...
            If chkBCBlanketApplied.Checked Then
                ...
                If BlanketCoinsuranceId <> "" Then
                    ...
                    Else
                        SetdropDownFromValue(ddBCCoInsurance, BlanketCoinsuranceId)
                    End If
                End If
                'If BlanketCoinsuranceId <> "" Then SetdropDownFromValue(ddBCCoInsurance, BlanketCoinsuranceId)
                ...
                'Updated 12/02/2021 for CPP Endorsements Task 67023 MLW
                PopulateBuildingCoverageDeductibleWithBlanket()
                ...
                '    '    SetDropDownValue_ForceDiamondValue(ddBCDeductible, BlanketDeductibleId, QuickQuoteClassName.QuickQuoteLocation, QuickQuotePropertyName.DeductibleId)
                '    'Else
                '    '    SetdropDownFromValue(ddBCDeductible, BlanketDeductibleId)
                '    'End If
                'End If
                'If BlanketDeductibleId <> "" Then SetdropDownFromValue(ddBCDeductible, BlanketDeductibleId)
            Else
                trBCBlanketInfoRow.Attributes.Add("style", "display:none")
            ...
            ' Co-Insurance
            'Updated 12/02/2021 for CPP Endorsements Task 67023 MLW
            If MyBuilding.CoinsuranceTypeId <> "" Then
                If IsQuoteReadOnly() Then
                    SetDropDownValue_ForceDiamondValue(ddBCCoInsurance, MyBuilding.CoinsuranceTypeId, QuickQuoteClassName.QuickQuoteBuilding, QuickQuotePropertyName.CoinsuranceTypeId)
                Else
                    SetdropDownFromValue(ddBCCoInsurance, MyBuilding.CoinsuranceTypeId)
                End If
            End If
            'If MyBuilding.CoinsuranceTypeId <> "" Then SetdropDownFromValue(ddBCCoInsurance, MyBuilding.CoinsuranceTypeId)
            ' Valuation
            If ValuationACVHelper.IsValuationACVAvailable(Quote) Then
            ...
            ' Deductible
            'Updated 12/02/2021 for CPP Endorsements Task 67023 MLW
            PopulateBuildingCoverageDeductible()
            'If MyBuilding.DeductibleId <> "" Then
            '    SetDropDownValue_ForceDiamondValue(ddBCDeductible, MyBuilding.DeductibleId, QuickQuoteClassName.QuickQuoteLocation, QuickQuotePropertyName.DeductibleId)
            '    'If IsQuoteReadOnly() Then
            '    '    SetDropDownValue_ForceDiamondValue(ddBCDeductible, MyBuilding.DeductibleId, QuickQuoteClassName.QuickQuoteLocation, QuickQuotePropertyName.DeductibleId)
            '    'Else
            '    '    SetdropDownFromValue(ddBCDeductible, MyBuilding.DeductibleId)
            '    'End If
            'End If
            'If MyBuilding.DeductibleId <> "" Then SetdropDownFromValue(ddBCDeductible, MyBuilding.DeductibleId)
            If LocationWindHailHelper.IsLocationWindHailAvailable(Quote) Then
        ...
        If HasCoverage(CPRBuildingCoverageType.BusinessIncomeCoverage) Or bldgQuote.HasBusinessIncomeALS Then
            ...
            If MyBuilding.BusinessIncomeCov_Limit <> "0" AndAlso MyBuilding.BusinessIncomeCov_Limit <> "1" AndAlso IsNumeric(MyBuilding.BusinessIncomeCov_Limit) Then
            ...
            If MyBuilding.BusinessIncomeCov_RiskTypeId IsNot Nothing AndAlso MyBuilding.BusinessIncomeCov_RiskTypeId <> "" Then
                ...
                Else
                    SetdropDownFromValue(ddBICRiskType, MyBuilding.BusinessIncomeCov_RiskTypeId)
                End If
                'SetdropDownFromValue(ddBICRiskType, MyBuilding.BusinessIncomeCov_RiskTypeId)
            Else
            ...
            If MyBuilding.BusinessIncomeCov_RiskTypeId <> "" Then
                ...
                End If
            End If
            'If MyBuilding.BusinessIncomeCov_RiskTypeId <> "" Then SetdropDownFromValue(ddBICRiskType, MyBuilding.BusinessIncomeCov_RiskTypeId)
            ' Use Specific Rates
        ...
        If HasCoverage(CPRBuildingCoverageType.PersonalPropertyCoverage) Then
            ...
            If chkPPCBlanketApplied.Checked Then
                ...
                If BlanketCoinsuranceId <> "" Then
                    ...
                    Else
                        SetdropDownFromValue(ddPPCCoinsurance, BlanketCoinsuranceId)
                    End If
                End If
                'If BlanketCoinsuranceId <> "" Then SetdropDownFromValue(ddPPCCoinsurance, BlanketCoinsuranceId)
                ...
                'Updated 12/02/2021 for CPP Endorsements Task 67023 MLW
                PopulatePersPropCovDeductibleWithBlanket()
                'If BlanketDeductibleId <> "" Then
                ...
                '    '    SetDropDownValue_ForceDiamondValue(ddPPCDeductible, BlanketDeductibleId, QuickQuoteClassName.QuickQuoteBuilding, QuickQuotePropertyName.PersPropCov_DeductibleId)
                '    'Else
                '    '    SetdropDownFromValue(ddPPCDeductible, BlanketDeductibleId)
                '    'End If
                'End If
                'If BlanketDeductibleId <> "" Then SetdropDownFromValue(ddPPCDeductible, BlanketDeductibleId)
            Else
                trPPCBlanketAppliedInfoRow.Attributes.Add("style", "display:none")
            ...
            If MyBuilding.PersPropCov_CauseOfLossTypeId <> "" Then SetdropDownFromValue(ddPPCCauseOfLoss, MyBuilding.PersPropCov_CauseOfLossTypeId)
            'Updated 12/02/2021 for CPP Endorsements Task 67023 MLW
            If MyBuilding.PersPropCov_CoinsuranceTypeId <> "" Then
                If IsQuoteReadOnly() Then
                ...
                Else
                    SetdropDownFromValue(ddPPCCoinsurance, MyBuilding.PersPropCov_CoinsuranceTypeId)
                End If
            End If
            ...
            'Updated 12/02/2021 for CPP Endorsements Task 67023 MLW
            PopulatePersPropCovDeductible()
            'If MyBuilding.PersPropCov_DeductibleId <> "" Then
            '    SetDropDownValue_ForceDiamondValue(ddPPCDeductible, MyBuilding.PersPropCov_DeductibleId, QuickQuoteClassName.QuickQuoteBuilding, QuickQuotePropertyName.PersPropCov_DeductibleId)
            '    'If IsQuoteReadOnly() Then
            '    '    SetDropDownValue_ForceDiamondValue(ddPPCDeductible, MyBuilding.PersPropCov_DeductibleId, QuickQuoteClassName.QuickQuoteBuilding, QuickQuotePropertyName.PersPropCov_DeductibleId)
            '    'Else
            '    '    SetdropDownFromValue(ddPPCDeductible, MyBuilding.PersPropCov_DeductibleId)
            '    'End If
            'End If
            'If MyBuilding.PersPropCov_DeductibleId <> "" Then SetdropDownFromValue(ddPPCDeductible, MyBuilding.PersPropCov_DeductibleId)
            If LocationWindHailHelper.IsLocationWindHailAvailable(Quote) Then
        ...
        If HasCoverage(CPRBuildingCoverageType.PersonalPropertyOfOthersCoverage) Then
            ...
            If chkPPOBlanketApplied.Checked Then
                ...
                If BlanketCoinsuranceId <> "" Then
                    ...
                    Else
                        SetdropDownFromValue(ddPPOCoinsurance, BlanketCoinsuranceId)
                    End If
                End If
                'If BlanketCoinsuranceId <> "" Then SetdropDownFromValue(ddPPOCoinsurance, BlanketCoinsuranceId)
                ...
                'Updated 12/02/2021 for CPP Endorsements Task 67023 MLW
                PopulatePersPropOfOthersDeductibleWithBlanket()
                'If BlanketDeductibleId <> "" Then
                ...
                '    '    SetDropDownValue_ForceDiamondValue(ddPPODeductible, BlanketDeductibleId, QuickQuoteClassName.QuickQuoteBuilding, QuickQuotePropertyName.PersPropCov_DeductibleId)
                '    'Else
                '    '    SetdropDownFromValue(ddPPODeductible, BlanketDeductibleId)
                '    'End If
                'End If
                'If BlanketDeductibleId <> "" Then SetdropDownFromValue(ddPPODeductible, BlanketDeductibleId)
            Else
                trPPOBlanketAppliedInfoRow.Attributes.Add("style", "display:none")
            ...
            If MyBuilding.PersPropOfOthers_CoinsuranceTypeId <> "" Then
                ...
                Else
                    SetdropDownFromValue(ddPPOCoinsurance, MyBuilding.PersPropOfOthers_CoinsuranceTypeId)
                End If
            End If
            'If MyBuilding.PersPropOfOthers_CoinsuranceTypeId <> "" Then SetdropDownFromValue(ddPPOCoinsurance, MyBuilding.PersPropOfOthers_CoinsuranceTypeId)
            If ValuationACVHelper.IsValuationACVAvailable(Quote) Then
            ...
            'Updated 12/02/2021 for CPP Endorsements Task 67023 MLW
            PopulatePersPropOfOthersDeductible()
            'If MyBuilding.PersPropOfOthers_DeductibleId <> "" Then
            '    SetDropDownValue_ForceDiamondValue(ddPPODeductible, MyBuilding.PersPropOfOthers_DeductibleId, QuickQuoteClassName.QuickQuoteBuilding, QuickQuotePropertyName.PersPropCov_DeductibleId)
            '    'If IsQuoteReadOnly() Then
            '    '    SetDropDownValue_ForceDiamondValue(ddPPODeductible, MyBuilding.PersPropOfOthers_DeductibleId, QuickQuoteClassName.QuickQuoteBuilding, QuickQuotePropertyName.PersPropCov_DeductibleId)
            '    'Else
            '    '    SetdropDownFromValue(ddPPODeductible, MyBuilding.PersPropOfOthers_DeductibleId)
            '    'End If
            'End If
            'If MyBuilding.PersPropOfOthers_DeductibleId <> "" Then SetdropDownFromValue(ddPPODeductible, MyBuilding.PersPropOfOthers_DeductibleId)
            If LocationWindHailHelper.IsLocationWindHailAvailable(Quote) Then
        ...
        'Added 11/24/2021 for CPP Endorsements Tasks 66977 and 67041 MLW
        If IsQuoteReadOnly() Then
                ...
                If IsNullEmptyorWhitespace(MyLocation.WindHailDeductibleLimitId) = False Then '0 = N/A
                    ...
                    Else
                        If MyLocation.WindHailDeductibleLimitId = "0" Then
                            ...
                            chkBCWindHail.Enabled = False
                        Else
                            If chkBCWindHail.Checked = False Then
                                chkBCWindHail.Enabled = True
                            End If
                        End If
                        trBCWindHailText.Attributes.Add("style", "display:none")
    ...
    '    ' Set the HasBlanket flag
    '    If Quote.HasBlanketBuilding OrElse Quote.HasBlanketBuildingAndContents OrElse Quote.HasBlanketContents Then HasBlanket = True

    '    ' Set the HasAgreedAmount flag
    '    ' Location 0 Building 0 IsAgreedAmount holds the default value for all the buildings
    '    If HasBlanket Then  ' Only going to have agreed amount if blanket is selected
    '        If Quote.Locations IsNot Nothing AndAlso Quote.Locations.HasItemAtIndex(0) AndAlso Quote.Locations(0).Buildings IsNot Nothing AndAlso Quote.Locations(0).Buildings.HasItemAtIndex(0) Then
    '            If Quote.HasBlanketBuilding OrElse Quote.HasBlanketBuildingAndContents Then
    ...
    '    If HasAgreedAmount Then
    '        ' ** Agreed Amount CHECKED on Coverages page
    '        ' Only disable and check the agreed amount box if the blanket checkbox is checked
    '        ' BUILDING COVERAGE
    '        If HasCoverage(CPRBuildingCoverageType.BuildingCoverage) Then
    '            If chkBCBlanketApplied.Checked Then
    '                chkBCAgreedAmount.Checked = HasAgreedAmount
    '                hdn_Agreed_BC.Value = HasAgreedAmount
    '            Else
    '                chkBCAgreedAmount.Checked = MyBuilding.IsAgreedValue
    '                hdn_Agreed_BC.Value = MyBuilding.IsAgreedValue
    '            End If
    ...
    '            If Quote.HasBlanketBuilding OrElse Quote.HasBlanketBuildingAndContents Then
    '                If chkBCBlanketApplied.Checked Then
    '                    ' Quote has Agreed Amount AND blanket type is blanket building or combined AND blanket is checked 
    '                    ' Check the Agreed Amount checkbox and disable it
    '                    chkBCAgreedAmount.Checked = True
    '                    hdn_Agreed_BC.Value = True
    '                    chkBCAgreedAmount.Attributes.Add("disabled", "True")
    '                Else
    '                    ' Quote has Agreed Amount AND blanket type is blanket building or combined AND blanket is NOT checked 
    '                    If LocationIndex = 0 AndAlso BuildingIndex = 0 Then
    '                        ' If building zero then always disable the agreed amount because it must be changed on the coverages page
    '                        chkBCAgreedAmount.Checked = True
    '                        hdn_Agreed_BC.Value = True
    '                        chkBCAgreedAmount.Attributes.Add("disabled", "true")
    '                    Else
    '                        ' Not building zero, enable the checkbox
    '                        chkBCAgreedAmount.Attributes.Remove("disabled")
    '                    End If
    '                End If
    '            Else
    '                ' Quote blanket type is Personal Property - set the agreed amount checkbox to whatever is saved on the building and enable it
    '                'chkBCAgreedAmount.Checked = MyBuilding.IsAgreedValue
    '                If HasBlanket Then
    '                    chkBCAgreedAmount.Attributes.Add("disabled", "True")
    '                    ' If has blanket and agreed amount then disable the agreed amount checkbox
    '                    'If Quote.HasBlanketBuilding OrElse Quote.HasBlanketBuildingAndContents Then
    '                    '    chkBCAgreedAmount.Attributes.Add("disabled", "True")
    '                    'Else
    '                    '    chkBCAgreedAmount.Attributes.Remove("disabled")
    '                    'End If
    '                Else
    '                    ' does not have blanket
    '                    If (LocationIndex = 0 AndAlso BuildingIndex = 0) AndAlso (Quote.HasBlanketBuilding OrElse Quote.HasBlanketBuildingAndContents) Then
    '                        ' Always disable on building 0 if blanket type is building or combined
    '                        chkBCAgreedAmount.Attributes.Add("disabled", "True")
    '                    Else
    '                        chkBCAgreedAmount.Attributes.Remove("disabled")
    '                    End If
    ...
    '            End If
    '        Else
    '            ' Quote does NOT have building coverage
    '            chkBCAgreedAmount.Attributes.Remove("disabled")
    '        End If
    ...
    '            ' Quote has personal property coverage
    '            If chkPPCBlanketApplied.Checked Then
    '                chkPPCAgreedAmount.Checked = HasAgreedAmount
    '                hdn_Agreed_PPC.Value = HasAgreedAmount
    '            Else
    '                chkPPCAgreedAmount.Checked = MyBuilding.PersPropCov_IsAgreedValue
    '                hdn_Agreed_PPC.Value = MyBuilding.PersPropCov_IsAgreedValue
    '            End If
    '            If Quote.HasBlanketContents OrElse Quote.HasBlanketBuildingAndContents Then
    '                If chkPPCBlanketApplied.Checked Then
    '                    ' Quote has Agreed Amount AND blanket type is blanket contents AND blanket is checked
    '                    ' Check the Agreed Amount checkbox and disable it
    '                    chkPPCAgreedAmount.Checked = True
    '                    hdn_Agreed_PPC.Value = True
    '                    chkPPCAgreedAmount.Attributes.Add("disabled", "True")
    '                Else
    '                    ' Quote has Agreed Amount AND blanket type is contents or combined AND blanket is NOT checked 
    '                    If LocationIndex = 0 AndAlso BuildingIndex = 0 Then
    '                        ' BUILDING 0
    '                        If Quote.HasBlanketContents Then
    '                            ' When blanket is property only, disable the checkbox because it must be changed on the coverages page
    '                            chkPPCAgreedAmount.Attributes.Add("disabled", "true")
    '                        Else
    '                            ' When blanket is combined, enable the checkbox
    '                            chkPPCAgreedAmount.Attributes.Remove("disabled")
    '                        End If
    '                        'chkPPCAgreedAmount.Checked = True
    '                    Else
    ...
    '                End If
    '            Else
    '                ' Quote blanket type is Buildingonly - set the agreed amount checkbox to whatever is saved on the personal property coverage and enable it
    '                'chkPPCAgreedAmount.Checked = MyBuilding.PersPropCov_IsAgreedValue
    '                If HasBlanket Then
    '                    ' Blanket is building only - enable the PPC Agreed Amount checkbox
    '                    chkPPCAgreedAmount.Attributes.Remove("disabled")
    '                Else
    '                    If (LocationIndex = 0 AndAlso BuildingIndex = 0) AndAlso (Quote.HasBlanketContents) Then
    '                        ' Always disable on building 0 if blanket type is contents
    '                        chkPPCAgreedAmount.Attributes.Add("disabled", "true")
    '                    Else
    '                        chkPPCAgreedAmount.Attributes.Remove("disabled")
    '                    End If
    ...
    '            End If
    '        Else
    '            ' Quote does NOT have personal property coverage
    '            chkPPCAgreedAmount.Attributes.Remove("disabled")
    '        End If
    '    Else
    '        ' Agreed Amount UNCHECKED on Coverages page
    ...
    '                hdn_Agreed_BC.Value = MyBuilding.IsAgreedValue
    '            End If
    '            ' Disable the agreed amount checkbox if this is building 0 and blanket is building or combined
    '            If (LocationIndex = 0 AndAlso BuildingIndex = 0) AndAlso (Quote.HasBlanketBuilding OrElse Quote.HasBlanketBuildingAndContents) Then
    '                chkBCAgreedAmount.Attributes.Add("disabled", "True")
    '            Else
    '                If chkBCBlanketApplied.Checked Then
    ...
    Private Function HasCoverage(ByVal CovType As CPRBuildingCoverageType) As Boolean
        ...
        Select Case CovType
            Case CPRBuildingCoverageType.BuildingCoverage
                ...
                If MyBuilding.Limit <> "" Then Return True
            ...
            Case CPRBuildingCoverageType.BusinessIncomeCoverage
                ...
                If MyBuilding.BusinessIncomeCov_Limit IsNot Nothing AndAlso MyBuilding.BusinessIncomeCov_Limit <> "" AndAlso MyBuilding.BusinessIncomeCov_Limit <> "N/A" AndAlso MyBuilding.BusinessIncomeCov_Limit <> "0" Then Return True
            ...
            Case CPRBuildingCoverageType.PersonalPropertyCoverage
                ...
                If MyBuilding.PersPropCov_PersonalPropertyLimit <> "" OrElse MyBuilding.PersPropCov_PropertyTypeId <> "" Then Return True
                Exit Select
            Case CPRBuildingCoverageType.PersonalPropertyOfOthersCoverage
                If chkPersonalPropertyOfOthers.Checked Then Return True
                If MyBuilding.PersPropOfOthers_PersonalPropertyLimit <> "" Then Return True
        ...
        If MyBuilding IsNot Nothing Then
            ...
            If RemoveBuildingLevelDeductibleHelper.IsRemoveBuildingLevelDeductibleAvailable(Quote) Then
                ...
                If BlanketHelper_CPR_CPP.Has_CPRCPP_Blanket(SubQuoteFirst) Then
                    deductible = BlanketHelper_CPR_CPP.GetBlanketDeductibleID(SubQuoteFirst)
                Else
                    deductible = MyLocation.DeductibleId
                End If
                bcDeductibleToUse = deductible
                ppcDeductibleToUse = deductible
                ppoDeductibleToUse = deductible
            Else
            ...
            End If
            MyBuilding.NumberOfStories = txtNumOfStories.Text
            ...
            If chkBusinessIncomeCoverage.Checked Then
                ...
                If Not SubQuoteFirst.HasBusinessIncomeALS Then
                    ...
                    If chkINFEarthquake.Checked Then
                        ...
                        'If IsNewCo() AndAlso chkBICEarthquake.Checked Then 'no longer sending earthquake deductible for Business Income per requirements
                        '    MyBuilding.BusinessIncomeCov_EarthquakeDeductibleId = ddEarthquakeDeductible.SelectedValue
                        'Else
                        '    MyBuilding.BusinessIncomeCov_EarthquakeDeductibleId = ""
                        'End If
                    Else
                    ...
                    End If
                    ' Waiting Period Type
                    ' Always set the waiting period to 72 hours when the QUOTE does not have Business Income ALS  Bug 44978 MGB 3/10/2020
    ...
    Public Overrides Sub ValidateControl(valArgs As VRValidationArgs)
        ...
        'NewCo fields
        If IsNewCo() Then
            If (IsNullEmptyorWhitespace(txtYearBuilt.Text) OrElse YearBuiltValidationHelper.ValidYearBuilt(txtYearBuilt.Text, Quote) = False) Then
                Me.ValidationHelper.AddError(txtYearBuilt, "Invalid Year Built", accordList)
            End If
            If IsNullEmptyorWhitespace(txtSquareFeet.Text) Then
            ...
            Else
                ...
                End If
            End If
            If IsNullEmptyorWhitespace(txtNumOfStories.Text) Then
                Me.ValidationHelper.AddError(txtNumOfStories, "Invalid Number of Stories", accordList)
            End If
        Else
            If IsNullEmptyorWhitespace(txtNumOfStories.Text) AndAlso MyBuilding.EarthquakeApplies AndAlso IsNullEmptyorWhitespace(MyBuilding.BusinessIncomeCov_Limit) = False Then
                Me.ValidationHelper.AddError(txtNumOfStories, "Number of Stories Required", accordList)
            End If
        End If
        ...
        If chkBuildingCoverage.Checked Then
            If RemoveBuildingLevelDeductibleHelper.IsRemoveBuildingLevelDeductibleAvailable(Quote) Then
                ...
                ElseIf txtBCBuildingLimit.Text.TryToGetInt32 <= MyLocation.Deductible.TryToGetInt32 Then
        ...
        If chkPersonalPropertyCoverage.Checked Then
            If RemoveBuildingLevelDeductibleHelper.IsRemoveBuildingLevelDeductibleAvailable(Quote) Then
                ...
                ElseIf txtPPCPersonalPropertyLimit.Text.TryToGetInt32 <= MyLocation.Deductible.TryToGetInt32 Then
        ...
        If chkPersonalPropertyOfOthers.Checked Then
            If RemoveBuildingLevelDeductibleHelper.IsRemoveBuildingLevelDeductibleAvailable(Quote) Then
                ...
                ElseIf txtPPOPersonalPropertyLimit.Text.TryToGetInt32 <= MyLocation.Deductible.TryToGetInt32 Then
        ...
        If LocationIndex = 0 AndAlso BuildingIndex = 0 Then
            ...
            If SubQuoteFirst IsNot Nothing Then
                If RemoveBuildingLevelDeductibleHelper.IsRemoveBuildingLevelDeductibleAvailable(Quote) Then
                    ...
                    'ElseIf SubQuoteFirst.HasBlanketContents Then
                    '    If chkPersonalPropertyCoverage.Checked Then
                    '        SetDropDownValue_ForceDiamondValue(ddPPCDeductible, BlanketDeductibleId, QuickQuoteClassName.QuickQuoteLocation, QuickQuotePropertyName.DeductibleId)
                    '    End If
                    'End If
                    'If SubQuoteFirst.HasBlanketBuildingAndContents OrElse SubQuoteFirst.HasBlanketBuilding Then
                    '    If chkBuildingCoverage.Checked Then
                    ...
                    '    If chkPersonalPropertyCoverage.Checked Then
                    '        'Me.ddPPCDeductible.SelectedValue = Quote.Locations(0).Buildings(0).PersPropCov_DeductibleId
                    '        SetDropDownValue_ForceDiamondValue(ddPPCDeductible, Quote.Locations(0).DeductibleId, QuickQuoteClassName.QuickQuoteLocation, QuickQuotePropertyName.DeductibleId)
                    '    End If
                    'End If
                Else
                    If SubQuoteFirst.HasBlanketBuildingAndContents OrElse SubQuoteFirst.HasBlanketBuilding Then
    ...
    Public Sub PopulateInflationGuard()
        ' Building Coverage
        If MyBuilding IsNot Nothing AndAlso HasCoverage(CPRBuildingCoverageType.BuildingCoverage) Then
            'Updated 12/02/2021 for CPP Endorsements Task 67023 MLW
            If IsQuoteReadOnly() Then
                SetDropDownValue_ForceDiamondValue(ddBCInflationGuard, MyBuilding.InflationGuardTypeId, QuickQuoteClassName.QuickQuoteBuilding, QuickQuotePropertyName.InflationGuardTypeId)
            Else
                ...
                If MyBuilding.InflationGuardTypeId <> "" Then
                    If MyBuilding.InflationGuardTypeId = "0" Then
                        'Updated 10/20/2022 for task 77527 MLW - will now default to 4 (id 2)
                        If CPR_InflationGuardHelper.IsInflationGuardNo2Available(Quote) Then
                            ddBCInflationGuard.SelectedValue = "2"
                        Else
                            ddBCInflationGuard.SelectedValue = "1"
    ...
    Public Sub PopulateNewCoFields()
        ...
        If MyBuilding.NumberOfStories <> "0" Then txtNumOfStories.Text = MyBuilding.NumberOfStories Else txtNumOfStories.Text = ""
        If IsNewCo() Then
            trYearBuilt.Attributes.Add("style", "display:''")
            'If MyBuilding.YearBuilt <> "0" Then txtYearBuilt.Text = MyBuilding.YearBuilt Else txtYearBuilt.Text = ""
            trSquareFeet.Attributes.Add("style", "display:''")
            'If MyBuilding.SquareFeet <> "0" Then txtSquareFeet.Text = MyBuilding.SquareFeet Else txtSquareFeet.Text = ""
            'If MyBuilding.NumberOfStories <> "0" Then txtNumOfStories.Text = MyBuilding.NumberOfStories Else txtNumOfStories.Text = ""
            If MyBuilding.EarthquakeApplies Then
                trEarthquakeDeductibleRow.Attributes.Add("style", "display:''")
                SetFromValue(ddEarthquakeDeductible, MyBuilding.EarthquakeDeductibleId)
            End If
            Me.tdNumberOfStories.InnerText = "*Number of Stories"
            Me.trPrefillInfoTextNewCo.Attributes.Add("style", "display:''")
            Me.trPrefillInfoTextOldCo.Attributes.Add("style", "display:none")
        Else
            If MyBuilding.EarthquakeApplies AndAlso IsNullEmptyorWhitespace(MyBuilding.BusinessIncomeCov_Limit) = False Then
                Me.tdNumberOfStories.InnerText = "*Number of Stories"
            Else
                Me.tdNumberOfStories.InnerText = "Number of Stories"
            End If
            ...
            'If MyBuilding.NumberOfStories <> "0" Then txtNumOfStories.Text = MyBuilding.NumberOfStories Else txtNumOfStories.Text = ""
            trEarthquakeDeductibleRow.Attributes.Add("style", "display:none")
            Me.trPrefillInfoTextOldCo.Attributes.Add("style", "display:''")
            Me.trPrefillInfoTextNewCo.Attributes.Add("style", "display:none")
        End If
    End Sub

    Public Sub PopulateBuildingCoverages()
        If Quote IsNot Nothing AndAlso MyBuilding IsNot Nothing Then
            LoadBuildingCoverageDeductibles()
            ...
            If bldgQuote IsNot Nothing Then
                ...
                If RemoveBuildingLevelDeductibleHelper.IsRemoveBuildingLevelDeductibleAvailable(Quote) Then
                    If bldgQuote.HasBlanketBuilding OrElse bldgQuote.HasBlanketBuildingAndContents OrElse bldgQuote.HasBlanketContents Then
                        If ((chkPPCBlanketApplied IsNot Nothing AndAlso chkPPCBlanketApplied.Checked = True) OrElse (chkPPCBlanketApplied IsNot Nothing AndAlso chkPPCBlanketApplied.Checked = True) OrElse (chkPPOBlanketApplied IsNot Nothing AndAlso chkPPOBlanketApplied.Checked = True)) Then
                            HasBlanket = True
                        End If
                    End If
                Else
                ...
                If HasCoverage(CPRBuildingCoverageType.BuildingCoverage) Then
                    If HasBlanket AndAlso MyBuilding.IsBuildingValIncludedInBlanketRating Then
                        PopulateBuildingCoverageDeductibleWithBlanket()
                    Else
                        PopulateBuildingCoverageDeductible()
                    End If
                ...
                If HasCoverage(CPRBuildingCoverageType.PersonalPropertyCoverage) Then
                    If HasBlanket AndAlso MyBuilding.PersPropCov_IncludedInBlanketCoverage Then
                        PopulatePersPropCovDeductibleWithBlanket()
                    Else
                        PopulatePersPropCovDeductible()
                    End If
                ...
                If HasCoverage(CPRBuildingCoverageType.PersonalPropertyOfOthersCoverage) Then
                    If HasBlanket AndAlso MyBuilding.PersPropOfOthers_IncludedInBlanketCoverage Then
                        PopulatePersPropOfOthersDeductibleWithBlanket()
                    Else
                        PopulatePersPropOfOthersDeductible()
                    End If
    ...
    Public Sub LoadBuildingCoverageDeductibles()
        'Building Coverage Deductible
        If RemoveBuildingLevelDeductibleHelper.IsRemoveBuildingLevelDeductibleAvailable(Quote) Then
            If IsQuoteReadOnly() Then
                QQHelper.LoadStaticDataOptionsDropDown(ddBCDeductible, QuickQuote.CommonMethods.QuickQuoteHelperClass.QuickQuoteClassName.QuickQuoteLocation, QuickQuote.CommonMethods.QuickQuoteHelperClass.QuickQuotePropertyName.DeductibleId, , Quote.LobType)
                'Personal Property Coverage Deductible
                QQHelper.LoadStaticDataOptionsDropDown(ddPPCDeductible, QuickQuote.CommonMethods.QuickQuoteHelperClass.QuickQuoteClassName.QuickQuoteBuilding, QuickQuote.CommonMethods.QuickQuoteHelperClass.QuickQuotePropertyName.PersPropCov_DeductibleId, , Quote.LobType)
                'Personal Property of Others Deductible
                QQHelper.LoadStaticDataOptionsDropDown(ddPPODeductible, QuickQuote.CommonMethods.QuickQuoteHelperClass.QuickQuoteClassName.QuickQuoteBuilding, QuickQuote.CommonMethods.QuickQuoteHelperClass.QuickQuotePropertyName.PersPropCov_DeductibleId, , Quote.LobType)
            Else
    ...
    Public Sub PopulateBuildingCoverageDeductibleWithBlanket()
        If BlanketDeductibleId <> "" Then
            SetDropDownValue_ForceDiamondValue(ddBCDeductible, BlanketDeductibleId, QuickQuoteClassName.QuickQuoteLocation, QuickQuotePropertyName.DeductibleId)
        End If
    End Sub

    Public Sub PopulateBuildingCoverageDeductible()
        If RemoveBuildingLevelDeductibleHelper.IsRemoveBuildingLevelDeductibleAvailable(Quote) Then
            If MyLocation.DeductibleId <> "" Then
                SetDropDownValue_ForceDiamondValue(ddBCDeductible, MyLocation.DeductibleId, QuickQuoteClassName.QuickQuoteLocation, QuickQuotePropertyName.DeductibleId)
            End If
        Else
            If MyBuilding.DeductibleId <> "" Then
                SetDropDownValue_ForceDiamondValue(ddBCDeductible, MyBuilding.DeductibleId, QuickQuoteClassName.QuickQuoteLocation, QuickQuotePropertyName.DeductibleId)
    ...
    Public Sub PopulatePersPropCovDeductibleWithBlanket()
        If BlanketDeductibleId <> "" Then
            SetDropDownValue_ForceDiamondValue(ddPPCDeductible, BlanketDeductibleId, QuickQuoteClassName.QuickQuoteBuilding, QuickQuotePropertyName.PersPropCov_DeductibleId)
        End If
    End Sub

    Public Sub PopulatePersPropCovDeductible()
        If RemoveBuildingLevelDeductibleHelper.IsRemoveBuildingLevelDeductibleAvailable(Quote) Then
            If MyLocation.DeductibleId <> "" Then
                SetDropDownValue_ForceDiamondValue(ddPPCDeductible, MyLocation.DeductibleId, QuickQuoteClassName.QuickQuoteLocation, QuickQuotePropertyName.DeductibleId)
            End If
        Else
            If MyBuilding.PersPropCov_DeductibleId <> "" Then
                SetDropDownValue_ForceDiamondValue(ddPPCDeductible, MyBuilding.PersPropCov_DeductibleId, QuickQuoteClassName.QuickQuoteBuilding, QuickQuotePropertyName.PersPropCov_DeductibleId)
    ...
    Public Sub PopulatePersPropOfOthersDeductibleWithBlanket()
        If BlanketDeductibleId <> "" Then
            SetDropDownValue_ForceDiamondValue(ddPPODeductible, BlanketDeductibleId, QuickQuoteClassName.QuickQuoteBuilding, QuickQuotePropertyName.PersPropCov_DeductibleId)
        End If
    End Sub

    Public Sub PopulatePersPropOfOthersDeductible()
        If RemoveBuildingLevelDeductibleHelper.IsRemoveBuildingLevelDeductibleAvailable(Quote) Then
            If MyLocation.DeductibleId <> "" Then
                SetDropDownValue_ForceDiamondValue(ddPPODeductible, MyLocation.DeductibleId, QuickQuoteClassName.QuickQuoteLocation, QuickQuotePropertyName.DeductibleId)
            End If
        Else
            If MyBuilding.PersPropOfOthers_DeductibleId <> "" Then
                SetDropDownValue_ForceDiamondValue(ddPPODeductible, MyBuilding.PersPropOfOthers_DeductibleId, QuickQuoteClassName.QuickQuoteBuilding, QuickQuotePropertyName.PersPropCov_DeductibleId)
