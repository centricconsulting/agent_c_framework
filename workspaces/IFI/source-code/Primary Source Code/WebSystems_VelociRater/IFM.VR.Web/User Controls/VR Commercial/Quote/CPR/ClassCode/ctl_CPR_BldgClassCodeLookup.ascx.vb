...
Public Class ctl_CPR_BldgClassCodeLookup
    ...
    Public Overrides Sub AddScriptWhenRendered()
        ...
        Me.VRScript.CreatePopupForm(Me.divMain.ClientID, "Class Code Lookup", 750, 550, True, True, False, Me.txtFilterValue.ClientID, "")

        'Me.VRScript.CreateJSBinding(Me.btnSearch.ClientID, "click", "Cpr.ClearBuildingCLassificationLookupForm();VRClassCode.PerformCPRBuildingClassCodeLookup(" & Me.BuildingIndex.ToString & "," & CInt(Me.Quote.LobType).ToString() & "," & pgmtype & ",'#" & Me.ddlFilterBy.ClientID & "','#" & Me.txtFilterValue.ClientID & "','#" + Me.divResults.ClientID & "','" & divCCInfo.ClientID & "','" & Me.hdnClassCode.ClientID & "','" & Me.hdnDIAClass_Id.ClientID & "','" & Me.hdnDescription.ClientID & "','" & Me.hdnPMAID.ClientID & "','" & Me.hdnPMAs.ClientID & "','" & Me.hdnGroupRate.ClientID & "','" & Me.hdnClassLimit.ClientID & "','" & Me.hdnFootNote.ClientID & "','" & txtClassCode.ClientID & "','" & txtDIA_Id.ClientID & "','" & txtDescription.ClientID & "','" & ddPMA.ClientID & "','" & txtGroupRate.ClientID & "','" & txtClassLimit.ClientID & "','" & divFootnote.ClientID & "'); return false;")
        Dim IsIneligibleMotelHotelCodes As Boolean = HotelMotelRemovedRisks.IsHotelMotelRemovedRisksAvailable(Quote)
        Me.VRScript.CreateJSBinding(Me.btnSearch.ClientID, "click", "VRClassCode.PerformCPRBuildingClassCodeLookup(" & Me.BuildingIndex.ToString & "," & CInt(Me.Quote.LobType).ToString() & "," & pgmtype & ",'#" & Me.ddlFilterBy.ClientID & "','#" & Me.txtFilterValue.ClientID & "','#" + Me.divResults.ClientID & "','" & IsIneligibleMotelHotelCodes & "'); return false;")
        Me.VRScript.CreateJSBinding(Me.btnApply.ClientID, "click", "Cpr.ApplyBuildingClassCode(" & Me.BuildingIndex.ToString & ");")
    ...
    Public Sub Show()
        ...
        Me.VRScript.ScrollToWithOffset(Me.txtFilterValue, -60, Me.Page)
    End Sub
    ...
    Private Sub btnCancel_Click(sender As Object, e As EventArgs) Handles btnCancel.Click
        Session("CCLookupPerformed_" & Session.SessionID.ToString) = BuildingIndex.ToString
        Hide()
    End Sub
