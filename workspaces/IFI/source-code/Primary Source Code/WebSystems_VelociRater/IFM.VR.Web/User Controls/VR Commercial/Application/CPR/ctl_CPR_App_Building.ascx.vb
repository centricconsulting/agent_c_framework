...
Public Class ctl_CPR_App_Building
    ...
    Public ReadOnly Property MyAiList As List(Of QuickQuoteAdditionalInterest)
        Get
            Dim AiList As List(Of QuickQuoteAdditionalInterest) = Nothing
            ' Added 6/7/18 to make this work with CPR and CPP
            ' AI's on CPR/CPP are supposed to be stored at the quote level but are actually being saved on the first building
            If Quote.AdditionalInterests IsNot Nothing AndAlso Quote.AdditionalInterests.Count > 0 Then
                ' If quote level add'l interests exists, use it
                Return Quote.AdditionalInterests
            ElseIf Quote.Locations IsNot Nothing AndAlso Quote.Locations.Count > 0 AndAlso Quote.Locations(0).Buildings IsNot Nothing AndAlso Quote.Locations(0).Buildings.Count > 0 Then
                ' If quote level AI's are not there, try building 0
                Return Quote.Locations(0).Buildings(0).AdditionalInterests
            Else
                ' No AI's found at quote or building 0, return the quote level ai's
                Return Quote.AdditionalInterests
            End If
            Return AiList
        End Get
    ...
    Private Sub LoadPayeeDDLs()
        ...
        ddlPersonalPropertyLimitOthersLossPayeeName.Items.Clear()
        ddlBusinessIncomeLimitLossPayeeName.Items.Clear()
        ...
        li3.Text = "N/A"
        li3.Value = ""
        ddlPersonalPropertyLimitOthersLossPayeeName.Items.Add(li3)
        Dim li4 As New ListItem()
        li4.Text = "N/A"
        li4.Value = ""
        ddlBusinessIncomeLimitLossPayeeName.Items.Add(li4)
        ...
        'ddlBuildingLimitLossPayeeName.Items.Add(li)
        'ddlPersonalPropertyLimitLossPayeeName.Items.Add(li)
        'ddlPersonalPropertyLimitOthersLossPayeeName.Items.Add(li)
        'ddlBusinessIncomeLimitLossPayeeName.Items.Add(li)

        If Quote IsNot Nothing AndAlso MyAiList IsNot Nothing Then
            ...
            For Each ai As QuickQuote.CommonObjects.QuickQuoteAdditionalInterest In MyAiList
                ...
                ddlPersonalPropertyLimitLossPayeeName.Items.Add(li2)
                li3 = New ListItem
                li3.Text = ai.Name.DisplayName
                li3.Value = ai.ListId
                ddlPersonalPropertyLimitOthersLossPayeeName.Items.Add(li3)
                li4 = New ListItem
                li4.Text = ai.Name.DisplayName
                li4.Value = ai.ListId
                ddlBusinessIncomeLimitLossPayeeName.Items.Add(li4)
    ...
    Public Overrides Sub Populate()
        If Quote IsNot Nothing AndAlso IsNewCo() Then
        ...
        If MyBuilding.IsNotNull Then
            ...
            lblPersonalPropertyLimit.Text = "Personal Property Limit: " & MyBuilding.PersPropCov_PersonalPropertyLimit
            lblPersonalPropertyLimitOthers.Text = "Personal Property Limit Of Others: " & MyBuilding.PersPropOfOthers_PersonalPropertyLimit
            lblBusinessIncomeLimit.Text = "Business Income Limit: " & MyBuilding.BusinessIncomeCov_Limit

            'added 6/8/2017; originally in ctl_CPR_App_Location
            If IsNewCo() Then
                'NewCo
                ' Square Feet
            ...
            If buildingAI IsNot Nothing AndAlso QQHelper.IsZeroPremium(MyBuilding.Limit) = False Then
                pnlBuildingLimit.Visible = True
            ...
            If persPropAI IsNot Nothing AndAlso QQHelper.IsZeroPremium(MyBuilding.PersPropCov_PersonalPropertyLimit) = False Then
                pnlPersonalPropertyLimit.Visible = True
                SetPersonalPropertyAdditionalInterestFieldsFromAI(persPropAI, setLossPayeeName:=True)
            End If

            'updated 6/7/2017 to just get 1 buildingAI and set everything that we can
            Dim persPropOthersAI As QuickQuote.CommonObjects.QuickQuoteAdditionalInterest = QuickQuote.CommonMethods.QuickQuoteHelperClass.QuickQuoteAdditionalInterestForDescription(MyBuilding.AdditionalInterests, "Personal property others", cloneAI:=False, matchType:=QuickQuote.CommonMethods.QuickQuoteHelperClass.TextMatchType.TextOnly_IgnoreCasing)
            If persPropOthersAI IsNot Nothing AndAlso QQHelper.IsZeroPremium(MyBuilding.PersPropOfOthers_PersonalPropertyLimit) = False Then
                pnlPersonalPropertyLimitOthers.Visible = True
                SetPersonalPropertyLimitOthersLossFieldsFromAI(persPropOthersAI, setLossPayeeName:=True)
            End If

            'updated 6/7/2017 to just get 1 persPropAI and set everything that we can
            Dim busIncomeAI As QuickQuote.CommonObjects.QuickQuoteAdditionalInterest = QuickQuote.CommonMethods.QuickQuoteHelperClass.QuickQuoteAdditionalInterestForDescription(MyBuilding.AdditionalInterests, "Business income limit", cloneAI:=False, matchType:=QuickQuote.CommonMethods.QuickQuoteHelperClass.TextMatchType.TextOnly_IgnoreCasing)
            If busIncomeAI IsNot Nothing AndAlso QQHelper.IsZeroPremium(MyBuilding.BusinessIncomeCov_Limit) = False Then
                pnlBusinessIncomeLimit.Visible = True
                SetBusinessIncomeLimitLossFieldsFromAI(busIncomeAI, setLossPayeeName:=True)
            End If
    ...
    Public Overrides Function Save() As Boolean
        ...
        If MyBuilding IsNot Nothing Then
            ...
            'save/remove persPropOthers AI
            If ddlPersonalPropertyLimitOthersLossPayeeName.SelectedIndex > 0 Then
                Dim persPropOthersAI As QuickQuote.CommonObjects.QuickQuoteAdditionalInterest = QuickQuote.CommonMethods.QuickQuoteHelperClass.QuickQuoteAdditionalInterestForDescription(MyBuilding.AdditionalInterests, "Personal property others", cloneAI:=False, matchType:=QuickQuote.CommonMethods.QuickQuoteHelperClass.TextMatchType.TextOnly_IgnoreCasing, returnNewIfNothing:=True)
                If persPropOthersAI IsNot Nothing Then
                    Dim isSameListId As Boolean = False
                    With persPropOthersAI
                        If .ListId = Me.ddlPersonalPropertyLimitOthersLossPayeeName.SelectedValue Then
                            isSameListId = True
                        Else
                            isSameListId = False 'redundant
                        End If
                        .ListId = Me.ddlPersonalPropertyLimitOthersLossPayeeName.SelectedValue
                        .TypeId = Me.ddlPersonalPropertyLimitOthersLossPayeeType.SelectedValue
                        Select Case Me.ddlPersonalPropertyLimitOthersATIMA.SelectedValue
                            Case "3"
                                .ATIMA = True
                                .ISAOA = True
                            Case "1"
                        ...
                        If Me.Quote IsNot Nothing AndAlso Me.Quote.AdditionalInterests IsNot Nothing AndAlso Me.Quote.AdditionalInterests.Count > 0 Then
                            Dim sourceAI As QuickQuote.CommonObjects.QuickQuoteAdditionalInterest = QuickQuote.CommonMethods.QuickQuoteHelperClass.QuickQuoteAdditionalInterestForListId(Me.Quote.AdditionalInterests, .ListId, cloneAI:=False, firstOrLastItem:=QuickQuote.CommonMethods.QuickQuoteHelperClass.FirstOrLast.First, matchType:=QuickQuote.CommonMethods.QuickQuoteHelperClass.TextMatchType.IntegerOrText_IgnoreCasing, returnNewIfNothing:=False)
                            If sourceAI IsNot Nothing Then
                                QQHelper.CopyQuickQuoteAdditionalInterestNameAddressEmailsAndPhones(sourceAI, persPropOthersAI)
                            End If
                        End If
                        'End If
                    End With
                End If
            Else
                QuickQuote.CommonMethods.QuickQuoteHelperClass.RemoveQuickQuoteAdditionalInterestsForDescription(MyBuilding.AdditionalInterests, "Personal property others", matchType:=QuickQuote.CommonMethods.QuickQuoteHelperClass.TextMatchType.TextOnly_IgnoreCasing)
            End If

            'save/remove buisnessIncomeLimit AI
            If ddlBusinessIncomeLimitLossPayeeName.SelectedIndex > 0 Then
                Dim buisnessIncomeLimit As QuickQuote.CommonObjects.QuickQuoteAdditionalInterest = QuickQuote.CommonMethods.QuickQuoteHelperClass.QuickQuoteAdditionalInterestForDescription(MyBuilding.AdditionalInterests, "Business income limit", cloneAI:=False, matchType:=QuickQuote.CommonMethods.QuickQuoteHelperClass.TextMatchType.TextOnly_IgnoreCasing, returnNewIfNothing:=True)
                If buisnessIncomeLimit IsNot Nothing Then
                    Dim isSameListId As Boolean = False
                    With buisnessIncomeLimit
                        If .ListId = Me.ddlBusinessIncomeLimitLossPayeeName.SelectedValue Then
                            isSameListId = True
                        Else
                            isSameListId = False 'redundant
                        End If
                        .ListId = Me.ddlBusinessIncomeLimitLossPayeeName.SelectedValue
                        .TypeId = Me.ddlBusinessIncomeLimitLossPayeeType.SelectedValue
                        Select Case Me.ddlBusinessIncomeLimitATIMA.SelectedValue
                            Case "3"
                                .ATIMA = True
                                .ISAOA = True
                            Case "1"
                        ...
                        If Me.Quote IsNot Nothing AndAlso Me.Quote.AdditionalInterests IsNot Nothing AndAlso Me.Quote.AdditionalInterests.Count > 0 Then
                            Dim sourceAI As QuickQuote.CommonObjects.QuickQuoteAdditionalInterest = QuickQuote.CommonMethods.QuickQuoteHelperClass.QuickQuoteAdditionalInterestForListId(Me.Quote.AdditionalInterests, .ListId, cloneAI:=False, firstOrLastItem:=QuickQuote.CommonMethods.QuickQuoteHelperClass.FirstOrLast.First, matchType:=QuickQuote.CommonMethods.QuickQuoteHelperClass.TextMatchType.IntegerOrText_IgnoreCasing, returnNewIfNothing:=False)
                            If sourceAI IsNot Nothing Then
                                QQHelper.CopyQuickQuoteAdditionalInterestNameAddressEmailsAndPhones(sourceAI, buisnessIncomeLimit)
                            End If
                        End If
                        'End If
                    End With
                End If
            Else
                QuickQuote.CommonMethods.QuickQuoteHelperClass.RemoveQuickQuoteAdditionalInterestsForDescription(MyBuilding.AdditionalInterests, "Business income limit", matchType:=QuickQuote.CommonMethods.QuickQuoteHelperClass.TextMatchType.TextOnly_IgnoreCasing)
            End If
        End If
    ...
    'added 12/19/2017
    Private Sub SetPersonalPropertyLimitOthersLossFieldsFromAI(ByVal buildingAI As QuickQuote.CommonObjects.QuickQuoteAdditionalInterest, Optional ByVal setLossPayeeName As Boolean = True)
        If buildingAI IsNot Nothing Then
            If setLossPayeeName = True Then
                If QQHelper.IsPositiveIntegerString(buildingAI.ListId) = True Then
                    If Me.ddlPersonalPropertyLimitOthersLossPayeeName.Items IsNot Nothing Then
                        If Me.ddlPersonalPropertyLimitOthersLossPayeeName.Items.Count > 0 AndAlso Me.ddlPersonalPropertyLimitOthersLossPayeeName.Items.FindByValue(buildingAI.ListId) IsNot Nothing Then
                            Me.ddlPersonalPropertyLimitOthersLossPayeeName.SelectedValue = buildingAI.ListId
                        Else
                            'add name to dropdown and set
            ...
            End If
            If QQHelper.IsPositiveIntegerString(buildingAI.TypeId) = True Then
                If Me.ddlPersonalPropertyLimitOthersLossPayeeType.Items IsNot Nothing Then
                    If Me.ddlPersonalPropertyLimitOthersLossPayeeType.Items.Count > 0 AndAlso Me.ddlPersonalPropertyLimitOthersLossPayeeType.Items.FindByValue(buildingAI.TypeId) IsNot Nothing Then
                        Me.ddlPersonalPropertyLimitOthersLossPayeeType.SelectedValue = buildingAI.TypeId
                    Else
                        'add type to dropdown and set
            ...
            End If
            If buildingAI.ATIMA = True AndAlso buildingAI.ISAOA = True Then
                Me.ddlPersonalPropertyLimitOthersATIMA.SelectedValue = "3"
            ElseIf buildingAI.ATIMA = True Then
                Me.ddlPersonalPropertyLimitOthersATIMA.SelectedValue = "1"
            ElseIf buildingAI.ISAOA = True Then
                Me.ddlPersonalPropertyLimitOthersATIMA.SelectedValue = "2"
            Else 'ElseIf buildingAI.ATIMA = False AndAlso buildingAI.ISAOA = False Then
                Me.ddlPersonalPropertyLimitOthersATIMA.SelectedValue = "0"
            End If
        End If
    End Sub
    Private Sub SetBusinessIncomeLimitLossFieldsFromAI(ByVal persPropAI As QuickQuote.CommonObjects.QuickQuoteAdditionalInterest, Optional ByVal setLossPayeeName As Boolean = True)
        If persPropAI IsNot Nothing Then
            If setLossPayeeName = True Then
                If QQHelper.IsPositiveIntegerString(persPropAI.ListId) = True Then
                    If Me.ddlBusinessIncomeLimitLossPayeeName.Items IsNot Nothing Then
                        If Me.ddlBusinessIncomeLimitLossPayeeName.Items.Count > 0 AndAlso Me.ddlBusinessIncomeLimitLossPayeeName.Items.FindByValue(persPropAI.ListId) IsNot Nothing Then
                            Me.ddlBusinessIncomeLimitLossPayeeName.SelectedValue = persPropAI.ListId
                        Else
                            'add name to dropdown and set
            ...
            End If
            If QQHelper.IsPositiveIntegerString(persPropAI.TypeId) = True Then
                If Me.ddlBusinessIncomeLimitLossPayeeType.Items IsNot Nothing Then
                    If Me.ddlBusinessIncomeLimitLossPayeeType.Items.Count > 0 AndAlso Me.ddlBusinessIncomeLimitLossPayeeType.Items.FindByValue(persPropAI.TypeId) IsNot Nothing Then
                        Me.ddlBusinessIncomeLimitLossPayeeType.SelectedValue = persPropAI.TypeId
                    Else
                        'add type to dropdown and set
            ...
            End If
            If persPropAI.ATIMA = True AndAlso persPropAI.ISAOA = True Then
                Me.ddlBusinessIncomeLimitATIMA.SelectedValue = "3"
            ElseIf persPropAI.ATIMA = True Then
                Me.ddlBusinessIncomeLimitATIMA.SelectedValue = "1"
            ElseIf persPropAI.ISAOA = True Then
                Me.ddlBusinessIncomeLimitATIMA.SelectedValue = "2"
            Else 'ElseIf persPropAI.ATIMA = False AndAlso persPropAI.ISAOA = False Then
                Me.ddlBusinessIncomeLimitATIMA.SelectedValue = "0"
            End If
        End If
    End Sub
    ...
    ''added 6/8/2017 from ctl_CPR_App_Location
