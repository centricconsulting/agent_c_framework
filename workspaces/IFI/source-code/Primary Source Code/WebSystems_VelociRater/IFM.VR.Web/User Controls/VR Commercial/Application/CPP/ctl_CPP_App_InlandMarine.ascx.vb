...
Public Class ctl_CPP_App_InlandMarine
    ...
    Private ReadOnly Property HasMotorTruckCargo() As Boolean
        Get
            If UnScheduledMotorTruckCargoHelper.IsUnScheduledMotorTruckCargoAvailable(Quote) Then
                If Not IsNullWhitespaceEmptyZeroOrNegative(GoverningStateQuote.MotorTruckCargoUnScheduledVehicleCatastropheLimit) Then Return True Else Return False
            Else
...
#Region "Methods and Functions"
    ...
    Public Overrides Sub AddScriptWhenRendered()
        ...
        If IsQuoteReadOnly() Then
            Me.VRScript.CreateAccordion(divCPPAppIM.ClientID, hdnAccord, "1")
            Me.VRScript.CreateAccordion(divBuildersRisk.ClientID, hdnBuildersRiskAccord, "1")
            Me.VRScript.CreateAccordion(divComputers.ClientID, hdnComputersAccord, "1")
            Me.VRScript.CreateAccordion(divContractors.ClientID, hdnContractorsAccord, "1")
            Me.VRScript.CreateAccordion(divFineArts.ClientID, hdnFineArtsAccord, "1")
            'Me.VRScript.CreateAccordion(divInstallationFloater.ClientID, hdnInstallationAccord, "1")
            If UnScheduledMotorTruckCargoHelper.IsUnScheduledMotorTruckCargoAvailable(Quote) Then
                Me.VRScript.CreateAccordion(divUnScheduledMotorTruckCargo.ClientID, hdnMotorTrucCargoAccord, "1")
            Else
                Me.VRScript.CreateAccordion(divMotorTruckCargo.ClientID, hdnMotorTrucCargoAccord, "1")
            End If
            Me.VRScript.CreateAccordion(divOwnersCargo.ClientID, hdnOwnersCargoAccord, "1")
            Me.VRScript.CreateAccordion(divSchedulePropertyFloater.ClientID, hdnScheduledPropertyAccord, "1")
            Me.VRScript.CreateAccordion(divSigns.ClientID, hdnSignsAccord, "1")
            Me.VRScript.CreateAccordion(divTransportation.ClientID, hdnTransportationAccord, "1")
        Else
            ...
            If UnScheduledMotorTruckCargoHelper.IsUnScheduledMotorTruckCargoAvailable(Quote) Then
                Me.VRScript.CreateAccordion(divUnScheduledMotorTruckCargo.ClientID, hdnMotorTrucCargoAccord, "0")
            Else
        ...
        If UnScheduledMotorTruckCargoHelper.IsUnScheduledMotorTruckCargoAvailable(Quote) Then
            Me.VRScript.StopEventPropagation(Me.lbSaveUnScheduledMotorTruckCargo.ClientID)
        Else
...
#Region "Save Routines"
    ...
    Public Overrides Function Save() As Boolean
        'Updated 12/09/2021 for CPP Endorsements Task 67310 MLW
        If IsOnAppPage Then
            SaveBuildersRisk()
            SaveComputers()
            SaveContractors()
    ...
    Private Sub SaveContractors()
        ...
        If rptContractors.Items IsNot Nothing AndAlso rptContractors.Items.Count > 0 Then
            ...
            For Each ri As RepeaterItem In rptContractors.Items
                ...
                If txtDscr.Text <> "" Then ContractorsCoverage.Description = txtDscr.Text
                ' Valuation
...
#Region "Populate Routines"
    ...
    Private Sub PopulateContractors()
        ...
        If HasContractors() Then
            ...
            If govStateQuote IsNot Nothing Then
                ceSchedCovs = GoverningStateQuote.ContractorsEquipmentScheduledCoverages
...
#End Region
    ...
    Private Sub DeleteContractorsItem(ByVal ndx As Integer)
        ...
        'If GoverningStateQuote.ContractorsEquipmentScheduledCoverages.HasItemAtIndex(ndx) Then
        '    GoverningStateQuote.ContractorsEquipmentScheduledCoverages.RemoveAt(ndx)
        '    Me.Populate()
        '    Save_FireSaveEvent(False)
        'End If
        'updated 10/22/2020 to make sure user doesn't delete their only scheduled item
        Dim govStateQuote As QuickQuote.CommonObjects.QuickQuoteObject = GoverningStateQuote()
        If govStateQuote IsNot Nothing Then
            If govStateQuote.ContractorsEquipmentScheduledCoverages.HasItemAtIndex(ndx) Then
                If govStateQuote.ContractorsEquipmentScheduledCoverages.Count = 1 Then
                    Me.ValidationHelper.AddError("You cannot delete all of your Contractors Scheduled Equipment from here.  Please return to the Quote side if you wish to delete this coverage.")
                Else
...
#Region "Events"
    ...
    Private Sub lnkSave_Click(sender As Object, e As EventArgs) Handles lnkSave.Click, lbSaveBuildersRisk.Click, lbSaveComputers.Click, lbSaveContractors.Click, lbSaveFineArts.Click, lbSaveMotorTruckCargo.Click, lbSaveUnScheduledMotorTruckCargo.Click, lbSaveOwnersCargo.Click, lbSaveScheduledProperty.Click, lbSaveSigns.Click, lbSaveTransportation.Click
        Save_FireSaveEvent()
        Exit Sub
    End Sub
...
#Region "Section Repeater ItemDataBound Events"
    ...
    Private Sub rptContractors_ItemDataBound(sender As Object, e As RepeaterItemEventArgs) Handles rptContractors.ItemDataBound
        ...
        'If ddValuation IsNot Nothing Then SetFromValue(ddValuation, CONT.ValuationMethodTypeId) 'Removed 12/30/2021, done below
        ...
        ' Valuation
        'Updated 12/30/2021 for CPP Endorsements Task 66800 MLW
        ...
        If ddValuation IsNot Nothing Then
            ...
            Using popupSpecial As New PopupMessageClass.PopupMessageObject(Me.Page, "Replacement cost is only permitted on equipment that is 5 years old or newer.", "Replacement Cost Message")
                With popupSpecial
                    ...
                    .width = 300
                    .height = 200
                    .AddControlToBindTo(ddValuation)
                    .divId = "ddValuationPopup"
    ...
    Private Sub btnContractorsAdd_Click(sender As Object, e As EventArgs) Handles btnContractorsAdd.Click
        Dim newItem As QuickQuote.CommonObjects.QuickQuoteContractorsEquipmentScheduledCoverage = Nothing
        ...
        newItem = New QuickQuote.CommonObjects.QuickQuoteContractorsEquipmentScheduledCoverage()
        GoverningStateQuote.ContractorsEquipmentScheduledCoverages.Add(newItem)
        Me.Populate()
        Save_FireSaveEvent(False)
