...
Public Class ctlHayBuilding
    ...
    Public Event ToggleExpandMoreLess()
    Public Event RaiseRefreshPeakSeason()
    Public Event SavePeakSeason()
    Public Event RaisePeakButtonPress(buttonPress As String)
    Public Event RaiseRowNumber(rowNumber As Integer)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        PersPropType = QuickQuoteScheduledPersonalPropertyCoverage.QuickQuoteScheduledPersonalPropertyCoverageType.Farm_F_Hay_in_Barn
    End Sub
                    ...
                    If PersPropRepeater.DataSource IsNot Nothing Then
                        ...
                        For Each child In ChildVrControls
                            If TypeOf child Is ctlFarmScheduledLimit Then
                                ...
                                c.TogglePeakSeason = TogglePeakSeason
                                c.Populate()
                            End If
                        Next
                    End If
    ...
    Protected Sub AttachCoverageControlEvents()
        For Each cntrl As RepeaterItem In PersPropRepeater.Items
            ...
            AddHandler increasedLimitControl.ToggleMoreLessPeak, AddressOf ToggleMoreLess
            AddHandler increasedLimitControl.RefreshAfterNewPeak, AddressOf RefreshAfterPeakSeasonAdd
            AddHandler increasedLimitControl.SaveBeforeNewPeak, AddressOf SaveBeforePeakSeasonAdd
            AddHandler increasedLimitControl.RaisePeakButtonPress, AddressOf PeakButtonPress
            AddHandler increasedLimitControl.RaiseCurrentRowNumber, AddressOf RaiseSelectRowNumber
        Next
    End Sub

    Private Sub RaiseSelectRowNumber(rowNumber As Integer)
        RaiseEvent RaiseRowNumber(rowNumber)
    End Sub

    Private Sub PeakButtonPress(button As String)
        RaiseEvent RaisePeakButtonPress(button)
    End Sub

    Private Sub RefreshAfterPeakSeasonAdd()
        RaiseEvent RaiseRefreshPeakSeason()
    End Sub

    Private Sub SaveBeforePeakSeasonAdd()
        RaiseEvent SavePeakSeason()
    End Sub
    ...
    Private Sub ToggleMoreLess()
        RaiseEvent ToggleExpandMoreLess()
    End Sub
    ...
    Public Overrides Sub ValidateControl(valArgs As VRValidationArgs)
        ...
        For Each cntrl As RepeaterItem In PersPropRepeater.Items
            Dim increasedLimitControl As ctlFarmScheduledLimit = cntrl.FindControl("ctlFarmScheduledLimit")
            increasedLimitControl.PersPropType = PersPropType
            increasedLimitControl.ValidatorGroupName = "Hay in Buildings"
        Next
